...
inline void CChattingServer::HandleCSPacket_REQ_LOGIN(CPacket* CSpacket, CPlayer* player)
{
	...
	if (wcschr(ID, L'\0') == NULL)
	{
		Disconnect(player->_sessionID);
		// LOG(L"FightGame", CSystemLog::DEBUG_LEVEL, L"%s[%d] ID is Wrong: %lld\n", _T(__FUNCTION__), __LINE__);
		// ::wprintf(L"%s[%d] ID is Wrong: %lld\n", _T(__FUNCTION__), __LINE__);
		return;
	}

	if (wcschr(nickname, L'\0') == NULL)
	{
		Disconnect(player->_sessionID);
		// LOG(L"FightGame", CSystemLog::DEBUG_LEVEL, L"%s[%d] Nickname is Wrong: %lld\n", _T(__FUNCTION__), __LINE__);
		// ::wprintf(L"%s[%d] Nickname is Wrong: %lld\n", _T(__FUNCTION__), __LINE__);
		return;
	}
